 ...
 * テクスチャ管理用の変数群をまとめたもの。
 */
public class ModchuModel_TextureDataBase  {
//public class ModchuModel_TextureData implements ModchuModel_ITextureEntityReplacePoint {
	...
	public ModchuModel_TextureBoxBase textureBox[];
	public int textureIndex[];
	public ModchuModel_ModelMultiBase textureModel[];
	...
	public ModchuModel_TextureDataBase(EntityLivingBase pEntity, ModchuModel_IModelCaps pCaps) {
		owner = pEntity;
		entityCaps = pCaps;
		...
		color = 12;
		contract = false;
		textureBox = new ModchuModel_TextureBoxBase[2];
		...
		textureModel = new ModchuModel_ModelMultiBase[3];
	}
	...
	protected boolean setTextureNamesServer() {
		// Server
		boolean lf = false;
		ModchuModel_TextureBoxServer lbox;
		if (textureBox[0] instanceof ModchuModel_TextureBoxServer) {
			lbox = (ModchuModel_TextureBoxServer)textureBox[0];
		...
		if (textureBox[1] instanceof ModchuModel_TextureBoxServer && owner != null) {
			lbox = (ModchuModel_TextureBoxServer)textureBox[1];
			if (lbox.localBox != null) {
				if (ModchuModel_HelperReplacePoint.isClient) {
					for (int i = 0; i < 4; i++) {
						ItemStack is = owner.getCurrentItemOrArmor(i + 1);
