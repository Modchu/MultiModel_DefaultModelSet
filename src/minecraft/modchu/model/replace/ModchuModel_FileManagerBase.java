 ...
 *
 */
public class ModchuModel_FileManagerBase {
	...
	public static void init() {
		...
		if (ModchuModel_HelperReplacePoint.isClient) {
			minecraftDir = Modchu_Main.getMinecraftDir();
		} else {
...
//			Modchu_Debug.Debug(String.format("getMincraftFile-file:%s", minecraftJar.getName()));
		...
		if (ModchuModel_HelperReplacePoint.isClient) {
			try {
				assetsDir = (File)Modchu_Reflect.getPrivateValue(Minecraft.class, Modchu_Main.mc, 42);
			} catch (Exception e) {
				e.printStackTrace();
