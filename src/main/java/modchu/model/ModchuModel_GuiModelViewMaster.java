...
import modchu.model.ModchuModel_GuiSmallButtonMaster;
import modchu.model.ModchuModel_Main;
import modchu.model.ModchuModel_TextureManagerBase;

public class ModchuModel_GuiModelViewMaster extends ModchuModel_GuiBaseMaster implements Modchu_IGuiModelViewMaster {
	...
	protected boolean useScaleChange;

	public ModchuModel_GuiModelViewMaster(HashMap<String, Object> map) {
		super(map);
	}
	...
	@Override
	public void reInit() {
		Modchu_Debug.mDebug("ModchuModel_GuiModelViewMaster reInit() 1 getTextureName()="+getTextureName());
		super.reInit();
		Modchu_Debug.mDebug("ModchuModel_GuiModelViewMaster reInit() 2 getTextureName()="+getTextureName());
		initDrawEntity();
		setTextureValue();
	...
	public void initGui() {
		...
		if (useScaleChange
			...
			if (!buttonParts && !buttonPlayer) {
				if (useScaleChange) buttonList.add(newInstanceButton(7, x + 75, y + 25, 75, 15, "ScaleChange"));
			}
		}
		if (buttonShowArmor) buttonList.add(newInstanceButton(20, x, y + 70, 75, 15, "showArmor"));
		if (((Modchu_IGuiModelView) base).getScale() == 0.0F) {
			((Modchu_IGuiModelView) base).setScale(0.9375F);
		}
		guiMode = true;
	...
	public void initDrawEntity() {
		try {
			...
			if (drawEntity != null) {
				Modchu_Debug.lDebug("ModchuModel_GuiModelViewMaster initDrawEntity modelData start ----------------------");
				ModchuModel_ModelDataBase modelData = ModchuModel_ModelDataMaster.instance.getPlayerData(drawEntity);
				Modchu_Debug.lDebug("ModchuModel_GuiModelViewMaster initDrawEntity modelData end. ----------------------");
				if (modelData != null) {
					modelData.setCapsValue(modelData.caps_freeVariable, "showMainModel", true);
					modelData.setCapsValue(modelData.caps_freeVariable, "showShadowAndFire", false);
					modelData.setCapsValue(modelData.caps_freeVariable, "showRenderName", false);
	...
	protected void resetFlagCheck(boolean debug, boolean errorDefault) {
		...
		if (textureResetFlag) {
			ModchuModel_ModelDataMaster.instance.allModelTextureReset(drawEntity);
	...
	public void mouseClickMove(int mouseX, int mouseY, int clickButton, long time) {
		...
		//Modchu_Debug.dDebug("mouseClickMove anyButtonClick=" + ModchuModel_GuiSmallButtonMaster.anyButtonClick + " allButtonOutOfRangeClick="+ModchuModel_GuiSmallButtonMaster.allButtonOutOfRangeClick, 1);
		int width = Modchu_AS.getInt(Modchu_AS.guiScreenWidth, base);
		int height = Modchu_AS.getInt(Modchu_AS.guiScreenHeight, base);
		if (prevMouseX != -999
				&& prevMouseY != -999
				&& time > 200
				&& ((!ModchuModel_GuiSmallButtonMaster.anyButtonClick
				&& ModchuModel_GuiSmallButtonMaster.allButtonOutOfRangeClick)
				| !displayButton)) {
	...
	public void selected(String textureName, String textureArmorName, int color, boolean armorMode) {
		...
		Modchu_Debug.mDebug("ModchuModel_GuiModelViewMaster selected textureName="+textureName+" armorMode="+armorMode);
		Modchu_Debug.mDebug("ModchuModel_GuiModelViewMaster selected getTextureName()="+getTextureName());
	}
