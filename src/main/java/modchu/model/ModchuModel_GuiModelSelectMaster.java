...
public class ModchuModel_GuiModelSelectMaster extends ModchuModel_GuiModelViewMaster {
	...
	private ModchuModel_ModelDataBase[] modelDatas;
	private boolean isRendering[];
	public boolean displayModels;
	public boolean armorMode;
	private boolean initLoading;

	public ModchuModel_GuiModelSelectMaster(HashMap<String, Object> map) {
		super(map);
	}
	...
	private void guiModelSelectMasterInit(HashMap<String, Object> map) {
		...
		for (int i = 0; i < i1; i++) {
			...
			modelDatas[i] = (ModchuModel_ModelDataBase) ModchuModel_ModelDataMaster.instance.getPlayerData(drawEntitys[i]);
	...
	public void initGui() {
		...
		//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster initGui page="+page);
		//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster initGui maxPage="+maxPage);
		if (page > 10) {
		...
		if (maxPage > 0
			...
			//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster initGui i3="+i3);
		...
		//ModchuModel_ModelDataBase modelData = (ModchuModel_ModelDataBase) ModchuModel_ModelDataBaseMaster.instance.getPlayerData(drawEntitys[i2]);
	...
	public void actionPerformed(Object guibutton) {
		...
		if (flag) {
			...
			//pageNext
				...
				//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster actionPerformed pageNext offsetSlot="+offsetSlot);
			}
			//page set
			else if (id > 1999) {
		...
		flag = id == 4
				| id == 5;
		if (flag) {
			//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster actionPerformed prevButtonPage 0 prevButtonPage="+prevButtonPage);
			prevButtonPage = checkPrevButtonPage(id == 5 ? prevButtonPage + 1 : prevButtonPage - 1);
			//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster actionPerformed prevButtonPage 1 prevButtonPage="+prevButtonPage);
			initGui();
			return;
		}
		if (!armorMode) {
		...
		if (id == 102 | id == 103) {
			...
			Modchu_AS.set(Modchu_AS.minecraftDisplayGuiScreen, Modchu_Main.newModchuCharacteristicObject("Modchu_GuiModelView", ModchuModel_GuiModelSelectMaster.class, popWorld, parentScreen, id == 103, getColor()));
	...
	private int checkPrevButtonPage(int i) {
		...
		//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster checkPrevButtonPage i="+i);
		return i;
	}
	...
	private void maxOffsetSlotCheack() {
		...
		for (int i = 0; i < size; i += maxCount) {
			//Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster maxOffsetSlotCheack i="+i);
			if (getTexturesNamber(i, getColor()) != -1) maxOffsetSlot = i;
		}
		Modchu_Debug.mDebug("ModchuModel_GuiModelSelectMaster maxOffsetSlotCheack maxOffsetSlot="+maxOffsetSlot);
	}
	...
	private int getMaxTexturesNamber(int i) {
		if (armorMode) return ModchuModel_Main.texturesArmorNamberList.size();
		List list = ModchuModel_Main.texturesNamberMap.containsKey(i) ? ModchuModel_Main.texturesNamberMap.get(i) : null;
	...
	private void drawModel(int i, int j, int x, int y, float f, int modelNamber) {
		...
		if (drawEntitySetFlag) {
			...
			setScale(ModchuModel_Main.getModelScale(drawEntitys[i2]));
	...
	private int getTexturesNamber(int i, int i1) {
		if (armorMode) {
			if (i >= ModchuModel_Main.texturesArmorNamberList.size()) return -1;
			return ModchuModel_Main.texturesArmorNamberList.get(i);
		} else {
			if (!ModchuModel_Main.texturesNamberMap.containsKey(i1)) return -1;
			List<Integer> list = ModchuModel_Main.texturesNamberMap.get(i1);
	...
	private void selected() {
		...
		ModchuModel_ModelDataBase modelData = (ModchuModel_ModelDataBase) ModchuModel_ModelDataMaster.instance.getPlayerData(drawEntitys[i2]);
